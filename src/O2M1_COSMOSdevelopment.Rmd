---
title: "O2M1: COSMOS package development -  script to run adjusted workflow"
output:
  html_document:
    keep_md: yes
    toc: yes
...
runID <- paste(gsub(" |-|:", "_", Sys.time()), "TPPactestNA_newNAweight_Fstat_without30uM_adjrev_viper_COSMOS_T47D", sep = "_")
```
...
load("data/2021_09_13_09_13_53_COSMOSinput_T47D_TPPFstat_no30uM_viper.RData")
```

## Define TPP input

```{r}
TPP_cosmos_input_final <- TPP_cosmos_input_final_Fstat # input object in correct format for TPP input
TPP_all_scores <- TPP_Fstatmono_without30uM_T47D # all scores to extarct stabilized and destabilized weights
...
Code for old footprinting results.

```{r}
# cell_line <- "UWB_1.289" #UWB1.289_BRCA1 #T47D
# timepoint <- "24h"
# concentration <- "4uM"
# compound <- "Olaparib"
#
# condition_long <- "UWB1.289_24h_4uM"
```

```{r}
# #transcriptomics
# transcriptomic_regulators <- empiricalzscore_act_transcriptomics %>%
#   filter(sample == condition_long & zscore != 0)
#
# if (nrow(transcriptomic_regulators) < 20) {
#   transcriptomic_regulators <- empiricalzscore_act_transcriptomics %>%
#   filter(sample == condition_long) %>%
#   group_by(sample) %>%
#   top_n(wt = abs(score), n = 20)
# }
#
# TF_input_COSMOS <- sign(transcriptomic_regulators$score)
# names(TF_input_COSMOS) <- paste0("X",
#   mapIds(org.Hs.eg.db, transcriptomic_regulators$protein, 'ENTREZID', 'SYMBOL'))
...
#   filter(sample == condition_long & abs(zscore) > 0.5)
#
...
### Option 1: High coverage hits, opt as NA

```{r}
# TPP_hits_highcoverage <- TPP_all_scores %>%
#   dplyr::group_by(representative) %>%
#   dplyr::mutate(number_strategies = n()) %>%
#   dplyr::filter(number_strategies >= 3) %>%
#   dplyr::distinct(representative, .keep_all = T) %>%
#   dplyr::mutate(signed_score = ifelse(mode == "destabilized" | mode =="decreased abundance", -1, 1))
#
# TPP_input_COSMOS <- TPP_hits_highcoverage$signed_score
# names(TPP_input_COSMOS) <- paste0("X",
#   mapIds(org.Hs.eg.db,  TPP_hits_highcoverage$gene_name, 'ENTREZID', 'SYMBOL'))
# TPP_input_COSMOS <- TPP_input_COSMOS[names(TPP_input_COSMOS)!="XNA"]
#
# TPP_input_COSMOS_tmp <- rep(NaN, length(TPP_input_COSMOS))
# names(TPP_input_COSMOS_tmp) <- names(TPP_input_COSMOS)
```

### Option 2: Derived activities in final input objects

```{r}
TPP_input_COSMOS_tmp <- as.numeric(TPP_cosmos_input_final$sign_summ)
...
# TPP_input_COSMOS_tmp <- TPP_input_COSMOS_tmp[!is.na(TPP_input_COSMOS_tmp)]
```
...
p_trans_vs_prot <- ggplot(trans_vs_prot, aes(
  x = rank_trans,
  y = rank_prot
)) +
  geom_point()
...
Old: check direction overlap of phospho data with transcriptomics and proteomics

```{r}
# transcriptomic_regulators_proteomics <- inner_join(proteomics_filtered, transcriptomic_regulators, by = c("GeneSymbol" = "protein"))
```


```{r}
# phospho_filtered <- phospho_ola %>%
...
# load("data/KSN.RData")
#
# relevant_psites_expressed <- filter(KSN, enzyme_genesymbol %in% phosphoproteomic_regulators$protein) %>%
#   inner_join(phospho_filtered, by = c("substrate_genesymbol" = "ID_cosmos"))
#
# proteomics_phosphoproteomics <- inner_join(relevant_psites_expressed, proteomics_filtered, by = "GeneSymbol")
#
# p_proteomics_phosphoproteomics <- ggplot(proteomics_phosphoproteomics, aes(x = dense_rank(logFC.x),
#                                              y = dense_rank(logFC.y))) +
#   geom_point()
# p_proteomics_phosphoproteomics
#
# psites_vs_all <- trans_vs_prot_direction %>%
#   filter(GeneSymbol %in% relevant_psites_expressed$GeneSymbol)
#
# p_psites_vs_all <- ggplot(psites_vs_all, aes(x = agreement)) +
#   geom_bar()
# p_psites_vs_all
```
...
```{r}
# relevant_psites_entrez <- relevant_psites_expressed %>%
#   mutate(CosmosID = paste0("X", EntrezID))
#
# psite_coverage <- relevant_psites_entrez %>%
#   mutate(inPKN = ifelse(CosmosID %in% meta_network_final_fw1$source| CosmosID %in% meta_network_final_fw1$target, "T", "F"))
#
# psite_coverage2 <- relevant_psites_entrez %>%
#   mutate(inPKN = ifelse(CosmosID %in% meta_network$source| CosmosID %in% meta_network$target, "T", "F"))
```
...
save(COSMOS_object_fw1, COSMOS_object_fw2, COSMOS_object_rev1, COSMOS_object_rev2,
  file = paste0("/Users/miraburtscher/Documents/gsk/O2_Network_modeling/results/", runID, ".RData")
)
```
